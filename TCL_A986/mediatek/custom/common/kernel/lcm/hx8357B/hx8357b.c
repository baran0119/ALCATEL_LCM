...
static void init_lcm_registers(void)
{
	...
	//  send_ctrl_cmd(0xc6);     //set RGB interface
	//  send_data_cmd(0x82);
	//   Delayms(10);

	  send_ctrl_cmd(0xC0);          //Set panel driving
	  send_data_cmd(0x14); //1
	  send_data_cmd(0x3b); //2
	  send_data_cmd(0x00); //3
	  send_data_cmd(0x12); //4
	  send_data_cmd(0x11); //5


	  send_ctrl_cmd(0xC8);          //Set Gamma
	  send_data_cmd(0x00); //1
	  send_data_cmd(0x26); //2
	  send_data_cmd(0x11); //3
	  send_data_cmd(0x20); //4
	  send_data_cmd(0x00); //5
	  send_data_cmd(0x08); //6
	  send_data_cmd(0x66); //7
	  send_data_cmd(0x15); //8
	  send_data_cmd(0x77); //9
	  send_data_cmd(0x02); //10
	  send_data_cmd(0x04); //11
	  send_data_cmd(0x00); //12

	  send_ctrl_cmd(0xD0);          //Set Power
	  send_data_cmd(0x44);          //DDVDH
	  send_data_cmd(0x41);
	  send_data_cmd(0x07);          //VREG1

	  send_ctrl_cmd(0xD1);          //Set VCOM
	  send_data_cmd(0x43);          //VCOMH  2d
	  send_data_cmd(0x0a);          //VCOML  16

	  send_ctrl_cmd(0xD2);          //Set power for normal mode
	  send_data_cmd(0x05);          //SAP05
	  send_data_cmd(0x12);          //DC10/00  12

	  send_ctrl_cmd(0xE9);          //Set Panel
	  send_data_cmd(0x09);  //    08 or 09
	  ...
	  send_ctrl_cmd(0xC5);       //set display frame
	  send_data_cmd(0x0a);   //70hz
	  MDELAY(10);

	  send_ctrl_cmd(0x29);           //set Display On
	  MDELAY(120);
#endif
}
...
static unsigned int lcm_compare_id(void)
{
	...
	param1 =read_data_cmd(); //manufacturer id   83

	unsigned int param2 =0;
	param2 = read_data_cmd(); //module/driver  id  57
