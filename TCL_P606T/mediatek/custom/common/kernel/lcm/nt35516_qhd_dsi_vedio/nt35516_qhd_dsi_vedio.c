...
{
	unsigned int  data_array[16];
	unsigned char buffer_be[1];
	unsigned char buffer_0a[1];

	data_array[0] = 0x00013700;
	dsi_set_cmdq(data_array, 1, 1);
	read_reg_v2(0xBE,buffer_be, 1);	
...
//#else
//    printk("xiewei lcm 0x0a is %x-----------------\n", buffer_0a[0]);
//	printk("xiewei lcm 0xbe is %x-----------------\n", buffer_be[0]);
	//printk("xiewei lcm 0xdb is %x-----------------\n", buffer_db[0]);
	//printk("xiewei 0xc5 is %x,%x,%x--------------\n", buffer_c5[0], buffer_c5[1] ,buffer_c5[2]);
//#endif
	if ((buffer_0a[0]==0x9C)&&(buffer_be[0]==0x57)){
		return false;
	}else{
		return true;
	}
...
#if defined(BUILD_LK) || defined(BUILD_UBOOT)	
#else
    printk("xiewei lcm_esd_recover  begin-----------------\n");
#endif	
...
#if defined(BUILD_LK) || defined(BUILD_UBOOT)	
#else
	printk("xiewei lcm_esd_recover  end-----------------\n");
#endif	
