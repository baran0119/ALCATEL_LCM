...
static unsigned int lcm_compare_id(void)
{
 ...
 /*
 #ifdef BUILD_LK
      printf("lcm  txd txd txd txd txd txd txd NT35510 driver-----pin number id is %x---------\n",id);
#else
      printk("lcm  txd txd txd txd txd txd txd NT35510 driver-----pin number id is %x---------\n",id);
#endif



#ifdef BUILD_LK
      printf("lcm NT35510 id is %x,%x,%x,%x--------------\n", buffer_vcom[0], buffer_vcom[1] ,buffer_vcom[2], buffer_vcom[3]);
#else
      printk("lcm NT35510 id is %x,%x,%x,%x--------------\n", buffer_vcom[0], buffer_vcom[1] ,buffer_vcom[2], buffer_vcom[3]);
#endif
*/

      if ((id==1)&&(id1==0x53)&&(id2==1)){
