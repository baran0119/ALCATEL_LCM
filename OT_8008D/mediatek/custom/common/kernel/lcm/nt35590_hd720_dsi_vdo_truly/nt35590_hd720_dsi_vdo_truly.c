...
#ifndef BUILD_LK
static struct workqueue_struct *lcm_workqueue = NULL;
static struct work_struct lcm_work;
extern int log_start_fb;
//int *lcm_read_registers(void);
#endif

char *lcm_read_registers(void);
...
//#if defined(MTK_WFD_SUPPORT)  baoqiang remove at 20130121
...
// add gamma value by baoqiang at 20121212
#if 1
//GAMMA 2.2 START
//R+
    data_array[0] = 0x00023902;
...
//B-
    ...
    dsi_set_cmdq(&data_array, 2, 1); 
//GAMMA2.2 END 20121211
#endif
//gamma add end baoqiang 20121212
    data_array[0] = 0x00023902;////CMD1 
    data_array[1] = 0x000000FF;                 
    dsi_set_cmdq(data_array, 2, 1);    
    data_array[0] = 0x00023902;
    ...
    MDELAY(120); 
// no need to open CABC, backlight control by BB. baoqiang modify at 20121212
...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		//1 Every lane speed
//modify by zhiping.liu for disable rf desense @start
		params->dsi.pll_select=1;
//modify by zhiping.liu for disable rf desense @end
#if 0
...
#endif
#endif
#if 1  //legen add for nvk driver ic
	params->dsi.CLK_HS_POST = 26;
	params->dsi.PLL_CLOCK = LCM_DSI_6589_PLL_CLOCK_312;// 286MHZ\u3002
...
{
	frame_num++;
	printk("[qbq]-----%s---frame_num=%d---\n",__func__,frame_num);
	if(frame_num==400){
		frame_num=0;
	
		printk("Begin to read registers!\n");
		//queue_work(lcm_workqueue, &lcm_work);
		lcm_read_registers();
...
char *lcm_read_registers(void)
{
	...
	//----------------------
	//Read value[9Ch,00h,ECC]+Error Report(4 Bytes)
	//----------------------
	read_reg_v2(0x0a,buffer,7);
	data_array[0] = 0x00023902;//CMD2,Page0  
    data_array[1] = 0x000000FF;                 
    dsi_set_cmdq(data_array, 2, 1);
	printk("read register 0x0a, the values are:\n");
	for(i=0;i<7;i++)
		printk(" %x  \n",buffer[i]);
	printk("register 0x0a read end!\n");
...
static unsigned int lcm_esd_check(void)
{
  	...
  	static unsigned int err_count=0, ucount_1=0, ucount_2=0;
	unsigned char  buffer[3];
  	unsigned char buffer_2[8]={0};
...
//baoqiang add end	
	...
	printk("[legen]---%s--0x36: %x------\n",__func__,buffer[0]);//legen
	if(buffer[0]==0x90)
	{
		//return FALSE;
		esd_result=0;
	...
	{			 
		...
		esd_result=1;
		ucount_1++;
		printk("%s: 36 register error times:%d\n",__func__,ucount_1);//legen
	}
//baoqiang add
	printk("[legen]---the value of register 0x0a:\n");//legen
	for(i=0;i<7;i++)
		printk("  %0x\n",buffer_2[i]);


	if(buffer_2[3] == 0x02) //Check data identifier of error report
	{
		if(buffer_2[4] & 0x02) //Check SOT sync error
			err_count ++;
		else
			err_count = 0;			
	}
	...
	if((buffer_2[0] != 0x9c) || (err_count >= 2) )
	{
		err_count = 0;
		esd_result=1;
		ucount_2++;
		printk("%s: 0a register error times:%d\n",__func__,ucount_2);//legen
	}
	else
	{
		esd_result=0;
   ...
   printk("%s: read0A_result=%d and ucount_1=%d, ucount_2=%d \n", __func__,esd_result,ucount_1,ucount_2);
...
{
	//lcm_init();  //baoqiang remove, as no need re-init two time 20130115
	lcm_resume();
...
LCM_DRIVER nt35590_hd720_dsi_vdo_truly_lcm_drv = 
{
	...
	.read_registers = lcm_read_registers,
#if (LCM_DSI_CMD_MODE)
    .update         = lcm_update,
#endif
    };
