...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	params->dsi.pll_div1=21;		// fref=26MHz, fvco=fref*(div1+1)	(div1=0~63, fvco=500MHZ~1GHz)
...
{	
	unsigned char buffer_db[2];
	unsigned char buffer_da[2];
	unsigned char buffer_dc[2];
	...
	array[0] = 0x00023700;
	dsi_set_cmdq(array, 1, 1);
	read_reg_v2(0xDA,buffer_da, 2);

	array[0] = 0x00023700;
	dsi_set_cmdq(array, 1, 1);
	read_reg_v2(0xDB, buffer_db, 2);
	
	array[0] = 0x00023700;
	dsi_set_cmdq(array, 1, 1);
	read_reg_v2(0xDC, buffer_dc, 2);	

	array[0]=0x00351500;
	dsi_set_cmdq(&array, 1, 1);
	
#ifdef BUILD_UBOOT
	printf("lcm 0xda is %x, %x--------------\n", buffer_da[0], buffer_da[1]);
	printf("lcm 0xdb is %x, %x--------------\n", buffer_db[0], buffer_db[1]);
	printf("lcm 0xdc is %x, %x--------------\n", buffer_dc[0], buffer_dc[1]);	
#endif
	if ((buffer_da[0] == 0x54)&&(buffer_db[0] == 0x80)&&(buffer_dc[0] == 0x66)){
		return 1;
	}else{
		return 0;
	}
