...
static struct LCM_setting_table lcm_initialization_setting[] = {
	...
	*/
      ...
      {0x35,	1,	{0x00}},
       {0xE0,	15,	{0x00, 0x20, 0x20, 0x0B, 0x10, 0x07, 0x42, 0x99, 0x4F, 0x07, 0x09,0x07, 0x23, 0x22, 0x0F}},
 
       {0xE1,	15,	{0x00, 0x1B, 0x1C, 0x08, 0x16, 0x08, 0x30, 0x66, 0x3D, 0x08, 0x0F,0x04, 0x1F, 0x1F, 0x0F}},

	{0xC0,	2,	{0x17, 0x15}},

	{0xC1,	1,	{0x41}},

	{0xC5,	2,	{0x00, 0x4D, 0x80}},
...
static unsigned int lcm_esd_check(void)
{
   ...
   //  printk("lcm 0xc5 is %x,%x,%x,%x--------------\n", buffer_vcom[0], buffer_vcom[1] ,buffer_vcom[2], buffer_vcom[3]);
     if ((buffer_vcom[0]==0x00)&&(buffer_vcom[1]==0x4D)&&(buffer_vcom[3]==0x4D)){
...
static unsigned int lcm_compare_id(void)
{
	...
	mt_set_gpio_dir(50,0);  //input
        MDELAY(5);
	id = mt_get_gpio_in(50) ;
    
#ifdef BUILD_UBOOT
      printf("lcm  tdt   ili9488 id is %x,%x,%x,%x--------------\n", buffer_vcom[0], buffer_vcom[1] ,buffer_vcom[2], buffer_vcom[3]);
#endif

      if ((buffer_vcom[3]==0x88 && id == 0x01)){

             
#ifdef BUILD_UBOOT
      printf("lcm  tdt ili9488 driver-----pin number id is %x---------\n",id);
#endif
...
// ---------------------------------------------------------------------------
LCM_DRIVER ili9488_dsi_6575_lcm_tdt_drv =
{
    .name			= "ili9488_dsi_tdt_beetle",
